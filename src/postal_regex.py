...
StreetAddress = collections.namedtuple('StreetAddress', 'house_number street city state zip_code')

def search_for_address(text):
    LINE1REGEX = re.compile(r'(?P<house_number>[0-9]{1,10}) (?P<street>[a-z ]+? (?P<street_suffix>[a-z]+))', re.IGNORECASE)
...
Google Maps API (which has a monthy fee) and the Twitter API (which needs to be
approved) before we can do anything else.
'''
