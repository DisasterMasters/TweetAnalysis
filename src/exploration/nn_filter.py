import csv
import os
import re
import sys
from datetime import datetime

from crisislex import Crisislex
from nn_tweetCleaning_predict import model_predict
from tqdm import tqdm

saved_data_path = r"/home/manny/Desktop/cleaning/test"

# enter argument as to which file the program will take to convert to test data
# typeoffile = sys.argv[1]

lex_counter = 0
total = 0
ml_counter = 0
rf_counter = 0

for typeoffile in ['utility', 'media', 'nonprofit', 'gov', 'env']:
    exclude = []
    # TEMP: base exclusion
    exclude_base = ['puerto rico', 'virgin islands', 'texas', 'houston', 'maria', 'jose', 'harvey', 'katrina',
                    'florida']
    # add cmd line denoting location and change data directories based on that
    # choose between utility, media, nonprofit, and government
    if typeoffile == 'utility':
        outfile = '/utility_data_(not_utility_source)'
    if typeoffile == 'media':
        outfile = '/media_data'
        # REMOVE IF NOT DEALING WITH FLORIDA TWEETS
        exclude = ['puerto rico', 'virgin islands', 'texas', 'houston', 'maria', 'jose', 'harvey', 'katrina']
        exclude = exclude_base.copy()
        # remove certain words depending on where data is from`
        exclude.remove('florida')
    if typeoffile == 'nonprofit':
        outfile = '/nonprofit_data'
    if typeoffile == 'gov':
        outfile = '/gov_data'
    if typeoffile == 'env':
        outfile = '/env_data'
    if len(sys.argv) > 2:
        outfile = sys.argv[2]
    # find directory containing files with data from tweet scraper
    # direc = "../DATA/florida_data/" + typeoffile + "_data"
    # direc = "./"
    # direc = r"/home/manny/PycharmProjects/TweetAnalysis/DATA/florida_data/utility_data"
    direc = r"/home/manny/PycharmProjects/TweetAnalysis/DATA/florida_data/" + typeoffile + "_data"


    print(direc, outfile)

    # open output file
    # file = io.open(outfile, "w", encoding="utf-8")

    # import crisislex to filter out tweets without crisislex keywords in them
    c_lex = Crisislex()
    lex = c_lex.lex

    flag = 0

    # walk through files
    for dirs, subdirs, files in os.walk(direc):
        for fname in tqdm(files):
            try:
                print(fname)
                counter = 0
                f = open(dirs + "/" + fname, "r")
                name = str(fname)
                # open csv file if found
                if fname.endswith(".csv"):
                    csv_f = csv.reader(f)
                # open txt file if found
                elif fname.endswith(".txt"):
                    csv_f = csv.reader(f, delimiter='|')
                header = next(csv_f, None)  # skip header, save for output
                for row in csv_f:
                    # skip empty row or row that is too short
                    if len(row) > 8:
                        # make sure tweet has word from crisislex
                        tweet = row[5]
                        link = row[8]

                        if fname[0] == '@' or fname == "FloridaMediaTweets.csv":
                            tweet = row[4]
                            link = row[9]
                        # deal with messy dates
                        date_text = row[1].split(' ')[0]
                        date_text = date_text.strip()
                        date = ''
                        if '/' in date_text:
                            date = datetime.strptime(date_text, '%m/%d/%Y')
                        elif '-' in date_text:
                            date = datetime.strptime(date_text, '%Y-%m-%d')
                        '''
                        prob dont need this
                        #make sure date is in september
                        start = datetime(year=2017, month=9, day=1)
                        end = datetime(year=2017, month=9, day=30)
                        '''
                        start = datetime(year=2017, month=9, day=1)
                        end = datetime(year=2017, month=9, day=30)

                        # split tweet row to make sure there are more than 3 words in it
                        tmp = tweet.split(' ')

                        # filters out tweets authored by utility company
                        if typeoffile == 'utility' and (row[0] in row[8]):
                            continue
                        if start < date < end and len(tmp) > 3:
                            clean_text = tweet
                            # strip extra whitespace
                            clean_text = clean_text.strip()
                            # remove http/s link
                            clean_text = re.sub(r'https?([^\s]+)', '', clean_text)
                            # remove __NEWLINE__
                            clean_text = re.sub(r'__NEWLINE__', '', clean_text)
                            # remove pic.twitter.com
                            clean_text = re.sub(r'pic.twitter.com([^\s]+)', '', clean_text)
                            # remove @handle
                            clean_text = re.sub(r'@([^\s]+)', '', clean_text)
                            # remove via @___
                            clean_text = re.sub(r'via @([^\s]+)', '', clean_text)
                            # put tweet, date, and permalink into unique separator for file
                            text = clean_text + '~+&$!sep779++' + date_text + '~+&$!sep779++' + link

                            with open(saved_data_path + outfile + '.csv', 'a', newline='') as csvfile:
                                spamwriter = csv.writer(csvfile)
                                spamwriter.writerow([clean_text, date_text, link])
                            total = total + 1
                            counter = counter + 1

                            # if any((txt in clean_text) for txt in lex) and not (any(txt in clean_text for txt in exclude)):
                            #     lex_counter = lex_counter + 1
                            #
                            #     with open(saved_data_path + outfile + '_lex.csv', 'a', newline='') as csvfile:
                            #         spamwriter = csv.writer(csvfile)
                            #         spamwriter.writerow([clean_text, date_text, link])
                            #
                            # try:
                            #     score = int(model_predict("CNN-non-static", clean_text))
                            #     if score >= 1:
                            #         ml_counter = ml_counter + 1
                            #
                            #         with open(saved_data_path + outfile + '_ml.csv', 'a', newline='') as csvfile:
                            #             spamwriter = csv.writer(csvfile)
                            #             spamwriter.writerow([clean_text, date_text, link])
                            # except:
                            #     pass
                            # tab separate each tweet line
                            text = text + "\t"
                            # decode text
                            try:
                                text = text.decode('utf-8', errors='ignore')
                            except:
                                pass
                            # file.write(text + '\n')
                            # if counter > 20:
                            #     print(counter)
                            #     break
            except:
                    pass
print("lex_counter", lex_counter)
print(" ml_counter", ml_counter)
print(" rf_counter", rf_counter)
print(" total", total)
